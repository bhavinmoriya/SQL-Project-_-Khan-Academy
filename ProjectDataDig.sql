...
group by Date
having Winner_Pts > 50;

select Date, Winner_Pts,
case
when Winner_Pts > 45 then "Good"
when Winner_Pts <= 45 and Winner_Pts > 20 then "Work Hard"  
when Winner_Pts = 20 then "Dude"
else "work very hard"
end as Remarks
from superbowls;

select Date, Winner from superbowls
where Winner_Pts > 50 and Winner = "Dallas Cowboys";

/*select Date, Winner, Loser,
case
when Winner = "Dallas Cowboys" then 'good'
else 'no worries'
end as Remarks
from superbowls;
...
select Date, Winner, Loser,max(Winner_Pts) from superbowls
